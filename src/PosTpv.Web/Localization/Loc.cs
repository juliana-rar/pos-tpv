using System.Globalization;

namespace PosTpv.Web.Localization;

/// <summary>
/// Tiny English-keyed localizer. UI code is written in English and each visible
/// string is wrapped with <see cref="T"/>; when the current UI culture is Spanish
/// the matching translation is returned, otherwise the English literal is used as
/// a safe fallback. This keeps translation friction low — no resource files, no
/// invented keys — while covering the whole application.
///
/// The active culture is set per request/circuit by the RequestLocalization
/// middleware (cookie-driven), so <see cref="CultureInfo.CurrentUICulture"/> is the
/// single source of truth here.
/// </summary>
public static class Loc
{
    public const string CookieName = "pos-culture";
    public static readonly string[] Supported = { "en", "es" };

    public static bool IsSpanish =>
        CultureInfo.CurrentUICulture.TwoLetterISOLanguageName.Equals("es", StringComparison.OrdinalIgnoreCase);

    /// <summary>Translate an English string to the current UI culture.</summary>
    public static string T(string english)
    {
        if (IsSpanish && _es.TryGetValue(english, out var es))
            return es;
        return english;
    }

    // English → Spanish. Anything not present falls back to the English literal.
    private static readonly Dictionary<string, string> _es = new(StringComparer.Ordinal)
    {
        // ---- Navigation / chrome ----
        ["Home"] = "Inicio",
        ["Dashboard"] = "Panel",
        ["Point of Sale"] = "Punto de venta",
        ["Orders"] = "Pedidos",
        ["Tables"] = "Mesas",
        ["Reservations"] = "Reservas",
        ["Today"] = "Hoy",
        ["Upcoming"] = "Próximas",
        ["Past"] = "Pasadas",
        ["Statistics"] = "Estadísticas",
        ["Total reservations"] = "Total de reservas",
        ["Seated rate"] = "Tasa de sentadas",
        ["Cancellation rate"] = "Tasa de cancelación",
        ["Avg. party size"] = "Comensales promedio",
        ["By status"] = "Por estado",
        ["Busiest tables"] = "Mesas más reservadas",
        ["No reservations yet."] = "Todavía no hay reservas.",
        ["Nothing here."] = "No hay nada acá.",
        ["Full reservations dashboard"] = "Panel completo de reservas",
        ["Kitchen"] = "Cocina",
        ["Billing"] = "Facturación",
        ["View report"] = "Ver informe",
        ["Management"] = "Gestión",
        ["Products"] = "Productos",
        ["Categories"] = "Categorías",
        ["Menu"] = "Menú",
        ["More options"] = "Más opciones",
        ["Hide sidebar"] = "Ocultar barra lateral",
        ["Show sidebar"] = "Mostrar barra lateral",
        ["Sign out"] = "Cerrar sesión",
        ["Toggle theme"] = "Cambiar tema",
        ["Language"] = "Idioma",
        ["Access denied"] = "Acceso denegado",
        ["Your role does not have permission to view this page."] = "Tu rol no tiene permiso para ver esta página.",
        ["Back to home"] = "Volver al inicio",

        // ---- Launcher / hub ----
        ["Welcome back"] = "Bienvenido de nuevo",
        ["Choose a module to get started"] = "Elige un módulo para empezar",
        ["Live sales overview and top products"] = "Resumen de ventas en vivo y productos más vendidos",
        ["Take orders and charge tables"] = "Toma pedidos y cobra mesas",
        ["Track and charge open orders"] = "Sigue y cobra los pedidos abiertos",
        ["Manage the floor plan and joins"] = "Gestiona el plano de sala y las uniones",
        ["Book and manage table reservations"] = "Reserva y gestiona las mesas",
        ["Floor plan, joins and reservations"] = "Plano de sala, uniones y reservas",
        ["Kitchen display for live tickets"] = "Pantalla de cocina para comandas en vivo",
        ["Revenue reports and invoices"] = "Informes de ingresos y facturas",
        ["Manage the product catalogue"] = "Gestiona el catálogo de productos",
        ["Organize products into categories"] = "Organiza los productos en categorías",
        ["Enter"] = "Entrar",

        // ---- Dashboard ----
        ["Sales today"] = "Ventas de hoy",
        ["Sales this month"] = "Ventas del mes",
        ["Orders today"] = "Pedidos de hoy",
        ["Open orders"] = "Pedidos abiertos",
        ["Occupied tables"] = "Mesas ocupadas",
        ["Reservations today"] = "Reservas de hoy",
        ["Top products"] = "Productos más vendidos",
        ["No sales recorded yet."] = "Aún no hay ventas registradas.",
        ["Product"] = "Producto",
        ["Qty"] = "Cant.",
        ["Revenue"] = "Ingresos",
        ["Loading…"] = "Cargando…",

        // ---- Login ----
        ["Sign in to your point of sale"] = "Inicia sesión en tu punto de venta",
        ["Username"] = "Usuario",
        ["PIN / Password"] = "PIN / Contraseña",
        ["Sign in"] = "Iniciar sesión",
        ["Invalid username or PIN."] = "Usuario o PIN no válidos.",
        ["Demo accounts (PIN"] = "Cuentas demo (PIN",

        // ---- POS ----
        ["Order"] = "Pedido",
        ["No open table."] = "No hay mesa abierta.",
        ["Select a table"] = "Seleccionar mesa",
        ["Unavailable"] = "No disponible",
        ["No products in this category."] = "No hay productos en esta categoría.",
        ["Tap a product to add it."] = "Toca un producto para añadirlo.",
        ["VAT"] = "IVA",
        ["Send to kitchen"] = "Enviar a cocina",
        ["Cash"] = "Efectivo",
        ["Card"] = "Tarjeta",
        ["Voucher"] = "Vale",
        ["Other"] = "Otro",
        ["Split"] = "Dividir",
        ["Open a table"] = "Abrir mesa",
        ["seats"] = "plazas",
        ["Base price"] = "Precio base",
        ["Quantity"] = "Cantidad",
        ["Extras"] = "Extras",
        ["Loading extras…"] = "Cargando extras…",
        ["No extras available for this product."] = "Este producto no tiene extras disponibles.",
        ["Comment"] = "Comentario",
        ["No onion, well done…"] = "Sin cebolla, muy hecho…",
        ["No onion, well done, extra sauce…"] = "Sin cebolla, muy hecho, salsa extra…",
        ["Cancel"] = "Cancelar",
        ["Add to order"] = "Añadir al pedido",
        ["Payment"] = "Pago",
        ["Total due"] = "Total a pagar",
        ["Split equally"] = "Dividir en partes iguales",
        ["Apply"] = "Aplicar",
        ["Remove"] = "Quitar",
        ["Add"] = "Añadir",
        ["Rest in cash"] = "Resto en efectivo",
        ["Rest in card"] = "Resto en tarjeta",
        ["Remaining"] = "Restante",
        ["Change"] = "Cambio",
        ["Fully paid"] = "Pagado completo",
        ["Tendered"] = "Entregado",
        ["Finalize"] = "Finalizar",
        ["Keyboard shortcuts"] = "Atajos de teclado",
        ["Select category"] = "Seleccionar categoría",
        ["Quantity of last line"] = "Cantidad de la última línea",
        ["Remove last line"] = "Quitar última línea",
        ["Open table selector"] = "Abrir selector de mesas",
        ["Send to kitchen (shortcut)"] = "Enviar a cocina",
        ["Charge (cash)"] = "Cobrar (efectivo)",
        ["Split payment"] = "Dividir pago",
        ["Finalize payment (in dialog)"] = "Finalizar pago (en diálogo)",
        ["Close dialog"] = "Cerrar diálogo",
        ["Toggle this help"] = "Mostrar/ocultar esta ayuda",
        ["Line comment"] = "Comentario de línea",
        ["Note for the kitchen"] = "Nota para la cocina",
        ["Save"] = "Guardar",
        ["Comment (title)"] = "Comentario",
        ["Remove (title)"] = "Quitar",
        // Order / item status labels
        ["Open"] = "Abierto",
        ["Sent"] = "Enviado",
        ["Cooking"] = "Cocinando",
        ["Preparing"] = "Preparando",
        ["Ready"] = "Listo",
        ["Delivered"] = "Entregado",
        ["Paid"] = "Pagado",
        ["Table"] = "Mesa",
        ["Editing table"] = "Editando mesa",

        // ---- Orders ----
        ["All statuses"] = "Todos los estados",
        ["Refresh"] = "Actualizar",
        ["per row"] = "por fila",
        ["items"] = "artículos",
        ["No open orders."] = "No hay pedidos abiertos.",
        ["InPreparation"] = "En preparación",
        ["Food"] = "Comida",
        ["Drinks"] = "Bebidas",
        ["First course"] = "Primer plato",
        ["Second course"] = "Segundo plato",
        ["Desserts"] = "Postres",
        ["Course"] = "Plato",
        ["Type"] = "Tipo",
        ["Serve"] = "Servir",
        ["Serve drinks"] = "Servir bebidas",
        ["Drinks served"] = "Bebidas servidas",
        ["Serve firsts"] = "Servir primeros",
        ["Firsts served"] = "Primeros servidos",
        ["Serve seconds"] = "Servir segundos",
        ["Seconds served"] = "Segundos servidos",
        ["Serve desserts"] = "Servir postres",
        ["Desserts served"] = "Postres servidos",
        ["Click to undo"] = "Clic para deshacer",
        ["Now"] = "Ahora",
        ["Served"] = "Servida",
        ["Mark as not served"] = "Marcar como no servida",
        ["Fire seconds"] = "Marchar segundos",
        ["Seconds fired"] = "Segundos marchados",
        ["Seconds prepared"] = "Segundos preparados",
        ["Tell the kitchen to start the second courses"] = "Avisa a cocina para empezar los segundos platos",
        ["Charge"] = "Cobrar",
        ["Select"] = "Seleccionar",
        ["Up"] = "Subir",
        ["Down"] = "Bajar",
        ["Move to previous course"] = "Mover al plato anterior",
        ["Move to next course"] = "Mover al plato siguiente",
        ["Previous order"] = "Comanda anterior",
        ["Next order"] = "Comanda siguiente",
        ["Ticket"] = "Ticket",
        ["Invitation"] = "Invitación",
        ["Tap a product to invite it (not charged)"] = "Toca un producto para invitarlo (no se cobra)",
        ["Invited — not charged"] = "Invitación — no se cobra",
        ["Edit order"] = "Editar comanda",
        ["Current lines"] = "Líneas actuales",
        ["Done"] = "Hecho",
        ["Send to kitchen and close"] = "Enviar a cocina y cerrar",
        ["Confirm and close"] = "Confirmar y cerrar",
        ["or split"] = "o dividir",
        ["Open drawer"] = "Abrir cajón",
        ["Lock drawer"] = "Bloquear cajón",
        ["Move table"] = "Mover mesa",
        ["Print bill"] = "Imprimir cuenta",

        // ---- Tables ----
        ["Floor plan"] = "Plano de sala",
        ["Free"] = "Libre",
        ["Occupied"] = "Ocupada",
        ["Reserved"] = "Reservada",
        ["Save layout"] = "Guardar distribución",
        ["selected"] = "seleccionadas",
        ["Join"] = "Unir",
        ["New table"] = "Nueva mesa",
        ["Join tables"] = "Unir mesas",
        ["Assign tables"] = "Asignar mesas",
        ["Click the occupied table whose order you want to move."] = "Haz clic en la mesa ocupada cuyo pedido quieres mover.",
        ["Reassign"] = "Reasignar",
        ["Choose a table"] = "Elige una mesa",
        ["Confirm"] = "Confirmar",
        ["Edit layout"] = "Editar distribución",
        ["Drag to move · corner handle to resize · top handle to rotate · ◻ to change shape · 🔒 to lock. Changes are saved when you press"] =
            "Arrastra para mover · tirador de esquina para redimensionar · tirador superior para rotar · ◻ para cambiar la forma · 🔒 para bloquear. Los cambios se guardan al pulsar",
        ["Change shape"] = "Cambiar forma",
        ["Tap two or more free tables to join them into one group that shares a single bill."] =
            "Toca dos o más mesas libres para unirlas en un grupo que comparte una sola cuenta.",
        ["Edit table"] = "Editar mesa",
        ["Name"] = "Nombre",
        ["Seats"] = "Plazas",
        ["Shape"] = "Forma",
        ["Joined"] = "Unida",
        ["Group name"] = "Nombre del grupo",
        ["Name this table"] = "Ponle nombre a esta mesa",
        ["Shares a bill with:"] = "Comparte cuenta con:",
        ["Delete"] = "Eliminar",
        ["Confirm deletion"] = "Confirmar eliminación",
        ["Rename table"] = "Renombrar mesa",
        ["This can't be undone."] = "Esta acción no se puede deshacer.",
        ["Separate"] = "Separar",
        ["No tables yet. Create one to start."] = "Aún no hay mesas. Crea una para empezar.",
        ["Lock / unlock"] = "Bloquear / desbloquear",
        ["Rotate"] = "Rotar",
        ["Resize"] = "Redimensionar",
        ["tables joined."] = "mesas unidas.",
        // Table shapes
        ["Square"] = "Cuadrada",
        ["Round"] = "Redonda",
        ["Rectangular"] = "Rectangular",
        ["Oval"] = "Ovalada",

        // ---- Kitchen ----
        ["Kitchen display"] = "Pantalla de cocina",
        ["● Live"] = "● En vivo",
        ["● Offline"] = "● Sin conexión",
        ["active"] = "activos",
        ["No orders in the kitchen. Enjoy the calm. ☕"] = "No hay pedidos en cocina. Disfruta la calma. ☕",
        ["Mark all ready"] = "Marcar todo listo",
        ["just now"] = "ahora mismo",
        ["Fire second courses"] = "Marchar segundos",
        ["Got it"] = "Recibido",

        // ---- Billing ----
        ["From"] = "Desde",
        ["To"] = "Hasta",
        ["VAT collected"] = "IVA recaudado",
        ["Invoices"] = "Facturas",
        ["Average ticket"] = "Ticket medio",
        ["Revenue by day"] = "Ingresos por día",
        ["No revenue in this range."] = "Sin ingresos en este rango.",
        ["Payment methods"] = "Métodos de pago",
        ["No payments in this range."] = "Sin pagos en este rango.",
        ["Invoice"] = "Factura",
        ["Method"] = "Método",
        ["Date"] = "Fecha",
        ["No invoices in this range."] = "Sin facturas en este rango.",
        ["Subtotal"] = "Subtotal",
        ["Qty"] = "Cant.",
        ["Unit price"] = "Precio unit.",
        ["Line total"] = "Total línea",
        ["Paid with"] = "Pagado con",

        // ---- Products ----
        ["Search…"] = "Buscar…",
        ["Search table…"] = "Buscar mesa…",
        ["Show all tables"] = "Ver todas las mesas",
        ["Show fewer tables"] = "Ver menos mesas",
        ["New product"] = "Nuevo producto",
        ["Available"] = "Disponible",
        ["Off"] = "Inactivo",
        ["Edit"] = "Editar",
        ["No products found."] = "No se encontraron productos.",
        ["Edit product"] = "Editar producto",
        ["Description"] = "Descripción",
        ["Category"] = "Categoría",
        ["Colour"] = "Color",
        ["Price"] = "Precio",
        ["VAT %"] = "IVA %",
        ["Prep. minutes"] = "Minutos de prep.",
        ["Display order"] = "Orden",
        ["Allergens"] = "Alérgenos",
        ["Manage allergens"] = "Gestionar alérgenos",
        ["New allergen"] = "Nuevo alérgeno",
        ["Edit allergen"] = "Editar alérgeno",
        ["No allergens yet."] = "Aún no hay alérgenos.",
        ["No allergens found."] = "No se encontraron alérgenos.",
        ["No allergens defined yet."] = "Aún no hay alérgenos definidos.",
        ["New extra"] = "Nuevo extra",
        ["Edit extra"] = "Editar extra",
        ["No extras yet."] = "Aún no hay extras.",
        ["No extras defined yet."] = "Aún no hay extras definidos.",
        ["Filters the product list below by category — pick which products get the extra."] = "Filtra la lista de productos de abajo por categoría — elige a qué productos se aplica el extra.",
        ["Pick a category above to see its products."] = "Elige una categoría arriba para ver sus productos.",
        ["No products yet."] = "Aún no hay productos.",
        ["Add one"] = "Añadir uno",
        ["Visible"] = "Visible",
        ["— select —"] = "— seleccionar —",
        ["Image"] = "Imagen",
        ["Choose image"] = "Elegir imagen",
        ["Remove image"] = "Quitar imagen",
        ["Unsupported image format."] = "Formato de imagen no compatible.",
        ["Image is too large (max 5 MB)."] = "La imagen es demasiado grande (máx. 5 MB).",

        // ---- Categories ----
        ["New category"] = "Nueva categoría",
        ["hidden"] = "oculta",
        ["Order:"] = "Orden:",
        ["No categories yet."] = "Aún no hay categorías.",
        ["No categories found."] = "No se encontraron categorías.",
        ["Edit category"] = "Editar categoría",
        ["Icon (emoji)"] = "Icono (emoji)",
        ["Hidden"] = "Oculta",
        ["Hide"] = "Ocultar",
        ["Show"] = "Mostrar",
        ["Move up"] = "Subir",
        ["Move down"] = "Bajar",
        ["Duplicate"] = "Duplicar",
        ["Move…"] = "Mover…",
        ["Move"] = "Mover",
        ["Move to category"] = "Mover a categoría",
        ["Select a category to manage its products."] = "Selecciona una categoría para gestionar sus productos.",
        ["Cannot delete a category that still has products."] = "No se puede eliminar una categoría que aún tiene productos.",
        ["Quick comments"] = "Comentarios rápidos",
        ["No quick comments yet."] = "Aún no hay comentarios rápidos.",
        ["Save the category first to add quick comments."] = "Guarda la categoría primero para añadir comentarios rápidos.",
        ["e.g. Well done, No cheese…"] = "ej. Muy hecho, Sin queso…",

        // ---- Reservations ----
        ["New reservation"] = "Nueva reserva",
        ["Reservation policy"] = "Política de reservas",
        ["Taking reservations"] = "Cómo se toman las reservas",
        ["Accepting reservations"] = "Se aceptan reservas",
        ["Phone only"] = "Solo por teléfono",
        ["Not accepting reservations"] = "No se aceptan reservas",
        ["Online booking"] = "Reserva por web",
        ["Coming soon"] = "Próximamente",
        ["Close"] = "Cerrar",
        ["Expand map"] = "Expandir mapa",
        ["Minimise map"] = "Minimizar mapa",
        ["Unassigned"] = "Sin asignar",
        ["Assign"] = "Asignar",
        ["Reassign"] = "Reasignar",
        ["Unassign"] = "Quitar mesas",
        ["Table settings"] = "Configurar mesas",
        ["Seat"] = "Sentar",
        ["Mark as left"] = "Marcar salida",
        ["Pick one or more tables on the plan…"] = "Elige una o más mesas en el plano…",
        ["Click one or more tables on the plan to assign them to the reservation of"] = "Haz clic en una o más mesas del plano para asignarlas a la reserva de",
        ["Done"] = "Listo",
        ["no table"] = "sin mesa",
        ["No reservations for this day."] = "No hay reservas para este día.",
        ["Edit reservation"] = "Editar reserva",
        ["Guest"] = "Cliente",
        ["Customer name"] = "Nombre",
        ["Phone"] = "Teléfono",
        ["Party size"] = "Comensales",
        ["Children"] = "Niños",
        ["Total"] = "Total",
        ["people"] = "personas",
        ["High chairs"] = "Tronas",
        ["When"] = "Cuándo",
        ["Time"] = "Hora",
        ["Duration (min)"] = "Duración (min)",
        ["seats selected"] = "asientos seleccionados",
        ["Details"] = "Detalles",
        ["Custom colour"] = "Color personalizado",
        ["Accent colour"] = "Color de acento",
        ["None"] = "Ninguno",
        ["Pick a colour"] = "Elegir un color",
        ["Or use an image as background"] = "O usa una imagen de fondo",
        ["Enter a valid hex colour, e.g. #2f4a68."] = "Introduce un color hexadecimal válido, ej. #2f4a68.",
        ["Floor style"] = "Estilo del suelo",
        ["Grid"] = "Cuadrícula",
        ["Wood"] = "Madera",
        ["Tile"] = "Baldosa",
        ["Concrete"] = "Cemento",
        ["Marble"] = "Mármol",
        ["Terracotta"] = "Terracota",
        ["Plants"] = "Plantas",
        ["Design elements"] = "Elementos de diseño",
        ["Potted plant"] = "Planta en maceta",
        ["Small plant"] = "Planta pequeña",
        ["Hanging plant"] = "Planta colgante",
        ["Bush"] = "Arbusto",
        ["Small tree"] = "Árbol pequeño",
        ["Fern"] = "Helecho",
        ["Wall"] = "Pared",
        ["Door"] = "Puerta",
        ["Bar counter"] = "Barra",
        ["Column"] = "Columna",
        ["Window"] = "Ventana",
        ["Zoom in"] = "Acercar",
        ["Zoom out"] = "Alejar",
        ["Reset zoom"] = "Restablecer zoom",
        ["Zones"] = "Zonas",
        ["Add zone"] = "Añadir zona",
        ["Name this zone"] = "Nombra esta zona",
        ["Zone name"] = "Nombre de la zona",
        ["Rename"] = "Renombrar",
        ["— none —"] = "— ninguna —",
        ["Status"] = "Estado",
        ["Comments"] = "Comentarios",
        ["Party"] = "Comensales",
        ["Service"] = "Servicio",
        ["Summary"] = "Resumen",
        ["Back"] = "Atrás",
        ["Next"] = "Siguiente",
        ["Create reservation"] = "Crear reserva",
        ["Save changes"] = "Guardar cambios",
        ["Reservation created"] = "Reserva creada",
        ["Changes saved"] = "Cambios guardados",
        ["Search country or code…"] = "Buscar país o prefijo…",
        ["No matches."] = "Sin resultados.",
        ["No date selected"] = "Sin fecha seleccionada",
        ["Enter the customer's name."] = "Falta poner el nombre.",
        ["Enter a phone number."] = "Falta poner el teléfono.",
        // Reservation status labels
        ["Pending"] = "Pendiente",
        ["Confirmed"] = "Confirmada",
        ["Seated"] = "Sentada",
        ["Finished"] = "Finalizada",
        ["Cancelled"] = "Cancelada",

        // ---- Settings ----
        ["Settings"] = "Configuración",
        ["Configure the app title, staff accounts and service hours."] = "Configura el título de la app, las cuentas del personal y el horario de servicio.",
        ["General"] = "General",
        ["Schedule"] = "Horario",
        ["App title"] = "Título de la app",
        ["This name appears in the sidebar, the login page and the browser tab."] = "Este nombre aparece en la barra lateral, la página de inicio de sesión y la pestaña del navegador.",
        ["Lunch"] = "Comida",
        ["Dinner"] = "Cena",
        ["Start"] = "Inicio",
        ["End"] = "Fin",
        ["Current service"] = "Servicio actual",
        ["Closed"] = "Cerrado",
        ["Service hours affect the dashboard's current-service badge and flag reservations booked outside them."] = "El horario de servicio afecta a la insignia de servicio actual del panel y marca las reservas hechas fuera de él.",
        ["This reservation falls outside the configured lunch/dinner hours."] = "Esta reserva está fuera del horario de comida/cena configurado.",
        ["New user"] = "Nuevo usuario",
        ["Edit user"] = "Editar usuario",
        ["Full name"] = "Nombre completo",
        ["Role"] = "Rol",
        ["Admin"] = "Administrador",
        ["Waiter"] = "Camarero",
        ["Cashier"] = "Cajero",
        ["Active"] = "Activo",
        ["Locked"] = "Bloqueado",
        ["Unlock"] = "Desbloquear",
        ["Leave blank to keep the current PIN"] = "Deja en blanco para mantener el PIN actual",
        ["This user has orders on record and can't be deleted. Deactivate it instead."] = "Este usuario tiene pedidos registrados y no se puede eliminar. Desactívalo en su lugar.",
        ["No users yet."] = "Aún no hay usuarios.",
        ["Manage the app title, staff accounts and service hours"] = "Gestiona el título de la app, las cuentas del personal y el horario de servicio",

        // ---- Suppliers / purchases / stock ----
        ["Suppliers"] = "Proveedores",
        ["Purchases"] = "Compras",
        ["Stock"] = "Stock",
        ["Manage suppliers, purchases and product stock levels."] = "Gestiona proveedores, compras y niveles de stock de productos.",
        ["New supplier"] = "Nuevo proveedor",
        ["Edit supplier"] = "Editar proveedor",
        ["Inactive"] = "Inactivo",
        ["documents"] = "documentos",
        ["purchases"] = "compras",
        ["No suppliers yet."] = "Aún no hay proveedores.",
        ["This supplier has purchases on record and cannot be deleted."] = "Este proveedor tiene compras registradas y no se puede eliminar.",
        ["New purchase"] = "Nueva compra",
        ["No purchases recorded yet."] = "Aún no hay compras registradas.",
        ["Supplier"] = "Proveedor",
        ["Reference"] = "Referencia",
        ["Lines"] = "Líneas",
        ["Adjust"] = "Ajustar",
        ["Adjust stock"] = "Ajustar stock",
        ["Contact person"] = "Persona de contacto",
        ["Tax ID"] = "NIF/CIF",
        ["Documents"] = "Documentos",
        ["No documents attached."] = "No hay documentos adjuntos.",
        ["Attach document"] = "Adjuntar documento",
        ["File is too large (max 10 MB)."] = "El archivo es demasiado grande (máx. 10 MB).",
        ["Select…"] = "Seleccionar…",
        ["Unit cost"] = "Coste unitario",
        ["Add line"] = "Añadir línea",
        ["New quantity"] = "Cantidad nueva",
        ["e.g. inventory count"] = "ej. recuento de inventario",
        ["A supplier is required."] = "Se requiere un proveedor.",
        ["Add at least one line."] = "Añade al menos una línea.",
        ["All categories"] = "Todas las categorías",
        ["Movement history"] = "Historial de movimientos",
        ["Last updated"] = "Última actualización",
        ["Never"] = "Nunca",
        ["Products tracked"] = "Productos controlados",
        ["Units in stock"] = "Unidades en stock",
        ["Out of stock"] = "Sin stock",
        ["Inventory value"] = "Valor del inventario",
        ["Stock by category"] = "Stock por categoría",
        ["units"] = "unidades",
        ["Value"] = "Valor",
        ["Export"] = "Exportar",
        ["Download"] = "Descargar",
        ["View movement history"] = "Ver historial de movimientos",
        ["No stock movements found."] = "No se han encontrado movimientos de stock.",
        ["Reason"] = "Motivo",
        ["Note"] = "Nota",
        ["Purchase"] = "Compra",
        ["Sale"] = "Venta",
        ["Adjustment"] = "Ajuste",

        // ---- Albarán scans ----
        ["Delivery notes"] = "Albaranes",
        ["All suppliers"] = "Todos los proveedores",
        ["All statuses"] = "Todos los estados",
        ["Processing"] = "Procesando",
        ["Needs review"] = "Por revisar",
        ["Validated"] = "Validado",
        ["Failed"] = "Fallido",
        ["Scan delivery note"] = "Escanear albarán",
        ["No delivery notes scanned yet."] = "Aún no se ha escaneado ningún albarán.",
        ["No delivery notes match these filters."] = "Ningún albarán coincide con estos filtros.",
        ["Clear filters"] = "Limpiar filtros",
        ["Number"] = "Número",
        ["Status"] = "Estado",
        ["View"] = "Ver",
        ["Take a photo or upload an image of the delivery note. A local AI model will read it; you'll be able to review and correct the data before saving."]
            = "Haz una foto o sube una imagen del albarán. Un modelo de IA local lo leerá; podrás revisar y corregir los datos antes de guardar.",
        ["Take photo"] = "Tomar foto",
        ["Choose file"] = "Elegir archivo",
        ["On mobile, \"Choose file\" also lets you pick from Google Drive, Photos or other apps installed on the device."]
            = "En el móvil, \"Elegir archivo\" también permite elegir desde Google Drive, Fotos u otras apps instaladas en el dispositivo.",
        ["Please choose an image file."] = "Elige un archivo de imagen.",
        ["Delivery note"] = "Albarán",
        ["Processing… this can take a few minutes."] = "Procesando… puede tardar unos minutos.",
        ["Unknown"] = "Desconocido",
        ["Unit price"] = "Precio unitario",
        ["Description"] = "Descripción",
        ["Save draft"] = "Guardar borrador",
        ["Retry"] = "Reintentar",
        ["Validate and create purchase"] = "Validar y crear compra",
        ["Select a supplier and a product for every line."] = "Selecciona un proveedor y un producto para cada línea.",

        // ---- Receipt / ticket design ----
        ["Legal / fiscal name"] = "Nombre fiscal",
        ["Printed as the receipt header. Leave blank to reuse the app title."] = "Se imprime como cabecera del ticket. Déjalo en blanco para reutilizar el título de la app.",
        ["Address"] = "Dirección",
        ["Footer message"] = "Mensaje de pie",
        ["e.g. Thank you for your visit!"] = "ej. ¡Gracias por su visita!",
        ["Paper width"] = "Ancho de papel",
        ["Show tax breakdown"] = "Mostrar desglose de impuestos",
        ["Preview"] = "Vista previa",
        ["Print"] = "Imprimir",
        ["Receipt"] = "Ticket",
        ["Tax"] = "Impuestos",
        ["Thank you!"] = "¡Gracias!",
    };
}
